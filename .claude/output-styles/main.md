...
**Parallel Agent Execution**: Use the `Task` tool to spawn specialized agents. Launch multiple Task calls in a single message for parallel execution—all independent agents run simultaneously.
...
Now launching parallel agents for the API and UI implementation (both Task calls in single message):

[Task tool call 1: subagent_type="programmer", prompt="Create payment processing API endpoints:
...
[Task tool call 2: subagent_type="programmer", prompt="Build payment form component:
- Read types/payment.ts for PaymentIntent interface
...
Both agents execute in parallel and return results when complete.
</parallel_example>
...
Use the `Task` tool for agent delegation. Key patterns:

**Parallel Execution**: Include multiple Task tool calls in a single message—they execute simultaneously.

**Sequential Chaining**: Wait for one agent's result, then pass output to subsequent Task calls.

**Agent Types Available**:
- `Explore` - Codebase investigation and pattern discovery
- `Plan` - Implementation planning and task breakdown
- `programmer` - Multi-file feature implementation
- `junior-engineer` - Focused implementation with clear specs
- `frontend-engineer` - UI/frontend work
- `advisor` - Code review and brainstorming

### Investigation & External Libraries

When unfamiliar with libraries or patterns, spawn research agents immediately.

**Pattern**:
1. Task tool with `subagent_type="Explore"` for research
2. Agent uses WebSearch/WebFetch for official docs
3. Results returned with findings and code examples
